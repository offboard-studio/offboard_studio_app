...
import createAiGenerateProjectDialog, { GeneratedArchitecture } from '../dialogs/ai-generate-project-dialog';
import './styles.scss';
...
function MenuBar(props: MenuBarProps) {
    ...
    /**
     * Callback for 'AI ile Proje Oluştur' under 'File' menu.
     * Asks the backend to generate a full architecture from a natural-language
     * prompt and loads the result into the current editor.
     */
    const generateProjectWithAi = async (_event: ClickEvent) => {
        try {
            const result = await createAiGenerateProjectDialog({}) as GeneratedArchitecture;
            ...
            editor.loadProject({
                editor: arch.editor,
                design: arch.design,
                dependencies: (arch.dependencies as never) ?? {},
                package: {
                    name: 'AI Generated Project',
                    version: '0.0.1',
                    description: result.explanation || 'Generated by Offboard Studio AI',
                    author: '',
                    image: '',
                } as never,
            }, 'ai_generated_project');
        } catch (err) {
            // User cancelled (react-modal-promise rejects on cancel) — no-op.
        }
    }
    ...
    const buildAndDownload = (_event: ClickEvent) => {
        ...
        if ( model) {
            const backend = import.meta.env.VITE_BACKEND_URL || 'https://offboard-studio-backend.vercel.app';
            const url = `${backend.replace(/\/$/, '')}/api/build`;
    ...
    return (
        <AppBar position="static" className='menu-bar' id='menu-bar'>
            <Toolbar >
                <img src={logo} alt="Offboard Studio" width='50px' style={{ marginRight: '1em' }} />
                <Menu
                    ...
                    <MenuItem onClick={generateProjectWithAi}>Generate with AI…</MenuItem>
                ...
                <Menu
                    ...
                    <MenuItem onClick={() => setBlock('basic.aicode')}>AI Code</MenuItem>
