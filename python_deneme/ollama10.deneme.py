...
def main(inputs, outputs, parameters, synchronise):
    ...
    def create_maximum_connections(self, node_models: Dict[str, Any], component_infos: List[ComponentInfo]) -> List[Dict[str, Any]]:
        ...
        # PHASE 3: Feedback and control connections
        logger.info("🔄 Phase 3: Creating feedback and control connections...")
        feedback_connections = self.create_feedback_connections(port_mappings, node_list)
        ...
        broadcast_connections = self.create_broadcast_connections(port_mappings, node_list)
        ...
        self.print_connection_summary(port_mappings, connections)
    ...
    def create_feedback_connections(self, port_mappings: Dict[str, Any], node_list: List[str]) -> List[Dict[str, Any]]:
        """Create feedback connections (from later components back to earlier ones)"""
        connections = []
        
        for i, target_node_id in enumerate(node_list):
            target_info = port_mappings[target_node_id]
            
            for j, source_node_id in enumerate(node_list):
                if j <= i:  # Only feedback from later to earlier components
                    continue
                ...
                for source_port in source_info["output_ports"]:
                    ...
                    for target_port in target_info["input_ports"]:
                        ...
                        # Check if this could be a feedback connection
                        if self.is_feedback_connection(source_port["label"], target_port["label"]):
                            connection = self.create_connection_object(
                                source_node_id, source_port,
                                target_node_id, target_port,
                                "feedback",
    ...
    def create_broadcast_connections(self, port_mappings: Dict[str, Any], node_list: List[str]) -> List[Dict[str, Any]]:
        """Create broadcast connections (one output to multiple inputs)"""
        connections = []
        
        for source_node_id in node_list:
            source_info = port_mappings[source_node_id]
            ...
            for source_port in source_info["output_ports"]:
                ...
                # Find ALL compatible unconnected input ports across ALL other nodes
                compatible_targets = []
                
                for target_node_id in node_list:
                    ...
                    for target_port in target_info["input_ports"]:
                        ...
                        if self.is_broadcast_compatible(source_port["label"], target_port["label"]):
                            compatibility = self.calculate_enhanced_port_compatibility(
                                source_port["label"], target_port["label"]
                            )
                ...
                # If we have multiple compatible targets, create broadcast connections
                if len(compatible_targets) > 1:
                    # Sort by compatibility and take the best ones
                    compatible_targets.sort(key=lambda x: x["compatibility"], reverse=True)
                    
                    # Create connections to top compatible targets
                    for i, target in enumerate(compatible_targets[:3]):  # Limit to 3 broadcasts
                        connection = self.create_connection_object(
                            source_node_id, source_port,
    ...
    def is_feedback_connection(self, source_label: str, target_label: str) -> bool:
        """Check if this could be a feedback connection"""
        source_lower = source_label.lower()
        target_lower = target_label.lower()
    ...
    def is_broadcast_compatible(self, source_label: str, target_label: str) -> bool:
        """Check if this port is suitable for broadcasting"""
        source_lower = source_label.lower()
        target_lower = target_label.lower()
        
        # Common broadcast scenarios
        broadcast_types = [
            ("enable", ["enable", "trigger", "start"]),
    ...
    def print_connection_summary(self, port_mappings: Dict[str, Any], connections: List[Dict[str, Any]]):
        """Print detailed connection summary"""
        logger.info("\n📊 CONNECTION SUMMARY:")
        logger.info("=" * 50)
        
        total_inputs = sum(len(info["input_ports"]) for info in port_mappings.values())
        total_outputs = sum(len(info["output_ports"]) for info in port_mappings.values())
        total_parameters = sum(len(info["parameter_ports"]) for info in port_mappings.values())
        
        connected_inputs = sum(sum(1 for p in info["input_ports"] if p["connected"]) for info in port_mappings.values())
        connected_outputs = sum(sum(1 for p in info["output_ports"] if p["connected"]) for info in port_mappings.values())
        ...
        logger.info(f"Total Ports: {total_inputs + total_outputs + total_parameters}")
        logger.info(f"  - Input ports: {total_inputs} (connected: {connected_inputs})")
        logger.info(f"  - Output ports: {total_outputs} (connected: {connected_outputs})")
        logger.info(f"  - Parameter ports: {total_parameters}")
        ...
        if total_inputs > 0:
            input_efficiency = (connected_inputs / total_inputs) * 100
            logger.info(f"\nInput Connection Efficiency: {input_efficiency:.1f}%")
        
        if total_outputs > 0:
            output_efficiency = (connected_outputs / total_outputs) * 100
    ...
    def create_component_dependency(self, comp_info: ComponentInfo) -> Dict[str, Any]:
        """Create component dependency structure with COMPLETE inner model and connections"""
        
        # Create inner model with proper structure
        inner_models = {}
        inner_connections = []
        
        # Track all inner model IDs for connections
        input_model_ids = []
        output_model_ids = []
        constant_model_ids = []
        code_model_id = None
        
        # Create input models with proper ports
        for i, input_name in enumerate(comp_info.inputs):
            input_id = self.create_unique_id()
            input_model_ids.append(input_id)
            
            output_port_id = self.create_unique_id()
            ...
            inner_models[input_id] = {
                ...
                "data": {"name": input_name},
                "_output_port_id": output_port_id  # Store for connection creation
            }
        
        # Create main code block with ALL ports
        if comp_info.code:
            code_model_id = self.create_unique_id()
            code_ports = []
            code_input_port_ids = {}
            code_output_port_ids = {}
            code_param_port_ids = {}
            
            # Add input ports to code block
            for i, input_name in enumerate(comp_info.inputs):
                input_port_id = self.create_unique_id()
                code_input_port_ids[input_name] = input_port_id
            ...
            # Add output ports to code block
            for i, output_name in enumerate(comp_info.outputs):
                output_port_id = self.create_unique_id()
                code_output_port_ids[output_name] = output_port_id
            ...
            # Add parameter ports to code block
            for i, param_name in enumerate(comp_info.parameters or []):
                param_port_id = self.create_unique_id()
                code_param_port_ids[param_name] = param_port_id
            ...
            inner_models[code_model_id] = {
                ...
                "_input_port_ids": code_input_port_ids,
                "_output_port_ids": code_output_port_ids,
                "_param_port_ids": code_param_port_ids
            }
        
        # Create output models with proper ports
        for i, output_name in enumerate(comp_info.outputs):
            output_id = self.create_unique_id()
            output_model_ids.append(output_id)
            ...
            inner_models[output_id] = {
                ...
                "data": {"name": output_name},
                "_input_port_id": input_port_id,  # Store for connection creation
                "_output_name": output_name  # Store output name for matching
            }
        
        # Create constants for parameters with proper ports
        for i, param_name in enumerate(comp_info.parameters or []):
            const_id = self.create_unique_id()
            constant_model_ids.append(const_id)
            
            output_port_id = self.create_unique_id()
            
            # Smart default values based on parameter name
            default_value = self.get_smart_default_value(param_name)
            ...
            inner_models[const_id] = {
                ...
                "_output_port_id": output_port_id,  # Store for connection creation
                "_param_name": param_name  # Store parameter name for matching
            }
        
        # NOW CREATE ALL INNER CONNECTIONS
        inner_connections = self.create_complete_inner_connections(
            inner_models, comp_info, 
            input_model_ids, output_model_ids, constant_model_ids, code_model_id
        )
        
        # Create complete dependency structure with inner connections
        inner_graph_blocks = []
        inner_graph_wires = []
        
        # Create graph blocks for each inner model
        for model_id, model in inner_models.items():
            block_data = {
                "id": model_id,
                "type": model["type"],
                "data": model["data"],
            ...
            inner_graph_blocks.append(block_data)
        
        # Create graph wires from inner connections
        for conn in inner_connections:
            source_model = inner_models[conn["source"]]
            target_model = inner_models[conn["target"]]
            
            # Find port names for the wire
            source_port_name = self.find_port_name_by_id(source_model, conn["sourcePort"])
            target_port_name = self.find_port_name_by_id(target_model, conn["targetPort"])
            
            wire = {
                "source": {
                    "block": conn["source"],
                    "port": source_port_name,
            ...
            inner_graph_wires.append(wire)
        
        # Create layers for inner model
        links_layer = {
            "id": self.create_unique_id(),
            "type": "diagram-links",
            "isSvg": True,
        ...
        return {
            ...
            "design": {
                "board": "Python3-Noetic",
                "graph": {
                    "blocks": inner_graph_blocks,
                    "wires": inner_graph_wires
                }
            },
            "dependencies": {},
            # Add the complete inner model structure
            "model": {
                "id": self.create_unique_id(),
                "locked": False,
                "offsetX": 0,
    ...
    def create_complete_inner_connections(self, inner_models: Dict[str, Any], comp_info: ComponentInfo,
                                        input_model_ids: List[str], output_model_ids: List[str], 
                                        constant_model_ids: List[str], code_model_id: str) -> List[Dict[str, Any]]:
        """Create ALL inner connections within the component dependency"""
        connections = []
        
        if not code_model_id:
            logger.warning("No code model found, skipping inner connections")
            return connections
        
        code_model = inner_models[code_model_id]
        
        logger.info(f"🔧 Creating inner connections for component: {comp_info.name}")
        
        # 1. Connect INPUT models to CODE model
        for i, input_model_id in enumerate(input_model_ids):
            input_model = inner_models[input_model_id]
            input_name = comp_info.inputs[i]
            
            # Find output port of input model
            input_output_port_id = input_model["_output_port_id"]
            
            # Find corresponding input port on code model
            code_input_port_id = code_model["_input_port_ids"].get(input_name)
            
            if code_input_port_id:
                connection = self.create_inner_connection(
                    input_model_id, input_output_port_id,
                    code_model_id, code_input_port_id,
                    input_model, code_model,
                    f"input_{input_name}_to_code"
                )
                connections.append(connection)
                
                # Update port links
                self.add_link_to_port(input_model, input_output_port_id, connection["id"])
                self.add_link_to_port(code_model, code_input_port_id, connection["id"])
                
                logger.info(f"   ✅ Connected input '{input_name}' to code")
        
        # 2. Connect CODE model to OUTPUT models  
        for i, output_model_id in enumerate(output_model_ids):
            output_model = inner_models[output_model_id]
            output_name = comp_info.outputs[i]
            
            # Find input port of output model
            output_input_port_id = output_model["_input_port_id"]
            
            # Find corresponding output port on code model
            code_output_port_id = code_model["_output_port_ids"].get(output_name)
            
            if code_output_port_id:
                connection = self.create_inner_connection(
                    code_model_id, code_output_port_id,
                    output_model_id, output_input_port_id,
                    code_model, output_model,
                    f"code_to_output_{output_name}"
                )
                connections.append(connection)
                
                # Update port links
                self.add_link_to_port(code_model, code_output_port_id, connection["id"])
                self.add_link_to_port(output_model, output_input_port_id, connection["id"])
                
                logger.info(f"   ✅ Connected code to output '{output_name}'")
        
        # 3. Connect CONSTANT models to CODE model parameters
        for const_model_id in constant_model_ids:
            const_model = inner_models[const_model_id]
            param_name = const_model["_param_name"]
            
            # Find output port of constant model
            const_output_port_id = const_model["_output_port_id"]
            
            # Find corresponding parameter port on code model
            code_param_port_id = code_model["_param_port_ids"].get(param_name)
            
            if code_param_port_id:
                connection = self.create_inner_connection(
                    const_model_id, const_output_port_id,
                    code_model_id, code_param_port_id,
                    const_model, code_model,
                    f"constant_{param_name}_to_code"
                )
                connections.append(connection)
                
                # Update port links
                self.add_link_to_port(const_model, const_output_port_id, connection["id"])
                self.add_link_to_port(code_model, code_param_port_id, connection["id"])
                
                logger.info(f"   ✅ Connected constant '{param_name}' to code")
        
        logger.info(f"🎯 Created {len(connections)} inner connections for {comp_info.name}")
        return connections

    def create_inner_connection(self, source_id: str, source_port_id: str, 
                            target_id: str, target_port_id: str,
                            source_model: Dict[str, Any], target_model: Dict[str, Any],
                            connection_name: str) -> Dict[str, Any]:
        """Create a single inner connection with proper structure"""
        connection_id = self.create_unique_id()
        
        # Find port coordinates
        source_port = self.find_port_by_id(source_model, source_port_id)
        target_port = self.find_port_by_id(target_model, target_port_id)
        
        if not source_port or not target_port:
            logger.warning(f"Could not find ports for connection {connection_name}")
            return None
    ...
    def find_port_by_id(self, model: Dict[str, Any], port_id: str) -> Dict[str, Any]:
        """Find port by ID in model"""
        for port in model.get("ports", []):
            if port["id"] == port_id:
                return port
        return None

    def find_port_name_by_id(self, model: Dict[str, Any], port_id: str) -> str:
        """Find port name by ID for graph wires"""
        port = self.find_port_by_id(model, port_id)
        if port:
            return port.get("name", port.get("label", "unknown"))
        return "unknown"

    def add_link_to_port(self, model: Dict[str, Any], port_id: str, connection_id: str):
        """Add connection ID to port's links array"""
        for port in model.get("ports", []):
            if port["id"] == port_id:
    ...
    def get_smart_default_value(self, param_name: str) -> str:
        """Get smart default values based on parameter name"""
        param_lower = param_name.lower()
        
        # Smart defaults based on common parameter patterns
        if any(word in param_lower for word in ["kernel", "size", "window"]):
            return "5"
        elif any(word in param_lower for word in ["threshold", "thresh"]):
            if "low" in param_lower:
                ...
                return "100"
        elif any(word in param_lower for word in ["blur", "sigma"]):
            return "1.5"
        elif any(word in param_lower for word in ["topic", "rostopic"]):
            return "/cmd_vel"
        elif any(word in param_lower for word in ["config", "path"]):
            return "/path/to/config"
        elif any(word in param_lower for word in ["weights", "model"]):
            return "/path/to/model.weights"
        elif any(word in param_lower for word in ["rate", "frequency"]):
            return "30"
        elif any(word in param_lower for word in ["length", "link"]):
            return "1.0,1.0,1.0"
        elif any(word in param_lower for word in ["position", "joint"]):
            return "0.0,0.0,0.0"
        else:
            return "default_value"

    # UPDATE the create_node_model method to ensure proper inner model structure
    def create_node_model(self, node_id: str, comp_info: ComponentInfo, x: int, y: int, dep_id: str) -> Dict[str, Any]:
        """Create a complete node model with fully connected inner model structure"""
        ...
        # Create complete inner model with ALL connections
        inner_model = self.create_fully_connected_inner_model(comp_info)
        ...
        return {
            ...
            "ports": ports,
            "data": {},
            "model": inner_model,  # This contains the fully connected inner model
            "info": {
                "name": comp_info.name,
                "version": "2.0.0",
                "description": comp_info.description,
    ...
    def create_fully_connected_inner_model(self, comp_info: ComponentInfo) -> Dict[str, Any]:
        """Create complete inner model with ALL required layers and FULL connectivity"""
        
        # Create inner nodes
        inner_nodes = {}
        inner_connections = []
        
        # Track model IDs for connection creation
        input_model_ids = []
        output_model_ids = []
        constant_model_ids = []
        code_model_id = None
        
        # Create input nodes with proper structure
        for i, input_name in enumerate(comp_info.inputs):
            input_id = self.create_unique_id()
            input_model_ids.append(input_id)
            ...
            inner_nodes[input_id] = {
                ...
                "data": {"name": input_name},
                "_output_port_id": port_id
            }
        
        # Create main code block with complete port structure
        if comp_info.code:
            code_model_id = self.create_unique_id()
            code_ports = []
            code_input_port_ids = {}
        ...
        for i, output_name in enumerate(comp_info.outputs):
            ...
            inner_nodes[output_id] = {
                ...
                "data": {"name": output_name},
                "_input_port_id": port_id,
                "_output_name": output_name
            }
        
        # Create constant nodes for parameters
        for i, param_name in enumerate(comp_info.parameters or []):
            const_id = self.create_unique_id()
            constant_model_ids.append(const_id)
        ...
        # Create ALL inner connections
        inner_connections = self.create_complete_inner_connections(
            inner_nodes, comp_info,
            input_model_ids, output_model_ids, constant_model_ids, code_model_id
        )
        ...
        links_layer = {
            ...
            "isSvg": True,
            "transformed": True,
            "models": {conn["id"]: conn for conn in inner_connections if conn}
        }
...
def main():
    ...
    test_prompts = [
        ...
        "Design a full-stack ROS 2-based robotic system architecture for a mobile manipulator in a dynamic warehouse. The robot must autonomously perform pick-and-place tasks using a 6-DOF arm on top of a mobile base. The system must:\n\n1. Receive real-time task commands (e.g., via MQTT or REST).\n2. Prioritize tasks dynamically (task scheduler).\n3. Navigate the warehouse using SLAM, obstacle avoidance (LIDAR + camera), and real-time replanning (Nav2).\n4. Detect and localize objects using YOLOv8 or DINOv2 + CLIP with a camera.\n5. Compute IK and control the manipulator using MoveIt 2.\n6. Synchronize movement of base and arm.\n7. Handle errors gracefully (e.g., object missing, motion failure).\n8. Log and visualize data with RViz or CesiumJS web dashboard.\n9. Support modular deployment with Docker + CI/CD + OTA updates.\n\nPlease output:\n- A list of functional modules and their responsibilities\n- Suggested ROS 2 nodes, with topics/services/actions for communication\n- Suggested libraries/tools per module\n- File/folder naming conventions and code organization (e.g., src/perception/yolo_node.py)\n- Which parts should be Python vs C++\n- Dockerfile and launch file organization\n- Optional multi-robot support (fleet layer abstraction)",
        "nav2 and moveit2 use for our  differiantial drive robot" 
    ]
    
    try:
        # Test with robotics prompt (has complex components)
        prompt = test_prompts[6]  # robotics
